...
nPE = prod(FOV);%floor(real_scan_time/TR);%floor(ScanTime/(TR)); %number of phase-encode lines acquired in entire scan
nPart = floor(real_scan_time/kernshift);%floor(ScanTime/(kernshift)); %number of partitions of a k-space sweep per reconstructed image
end
