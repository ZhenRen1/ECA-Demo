...
ObsMask = double(ObsMask);
% weights stretched over time axis
weights_stretch = repmat(ReconPars.weights,1,1,1,ReconPars.nimage);
% v0 = repmat(KspaceSignal(:,:,:,1),1,1,1,ReconPars.nimage);
% v0 =  v0(:);
v0 = zeros(size(ObsMask));
v0(:,:,:,1) = KspaceSignal(:,:,:,1);
v0 = v0(:);

v0 = reshape(ObsMask.*(fft3d((ifft3d(...
    reshape(reshape(v0,[],ReconPars.nimage)/Dmat,...
...
mult_by_matrix = @(v) ...
    reshape(ObsMask.*(fft3d((ifft3d(...
    reshape(reshape(v,[],ReconPars.nimage)/Dmat,...
    AcqPars.nx,AcqPars.ny,AcqPars.nz,[])))./weights_stretch)),[],1);

% [v,flag,relres,iter,resnorms] = pcg(mult_by_matrix,KspaceSignal_obs(:),...
%     ReconPars.convergethresh,ReconPars.maxiter,[],[],v0);
% 
[v,flag,relres,iter,resnorms] = pcg(mult_by_matrix,KspaceSignal_obs(:),...
